...
s = p(1);
sigma = p(2);
% counter, beta, z, multipler, auxiliary variable, cost and constraint function
k = 1;
beta = self.Option.LineSearch.betaInit;
z = z_Init;
gamma_h = ones(self.NLP.Dim.h, 1);
gamma_c = ones(self.NLP.Dim.c, 1);
xi_c = ones(self.NLP.Dim.c, 1);
...
while true
    ...
    PSI = full(self.FuncObj.PSI(gamma_c, xi_c, sigma));
    PSI_grad_gamma_c = sparse(self.FuncObj.PSI_grad_gamma_c(gamma_c, xi_c, sigma));
    PSI_grad_xi_c = sparse(self.FuncObj.PSI_grad_xi_c(gamma_c, xi_c, sigma));
    ...
    KKT_Residual = [LAG_grad_z'; h; c - xi_c; PSI]; 
    % KKT matrix
    KKT_Matrix = self.evaluate_KKT_Matrix(h_grad, c_grad, LAG_hessian, PSI_grad_gamma_c, PSI_grad_xi_c);
    ...
    dxi_c    = dY(Y_Node(3) + 1 : Y_Node(4), 1); 
    % dYNorm (L_inf norm)
    dYNorm = norm(dY, inf);
    ...
    [z_k, gamma_h_k, gamma_c_k, xi_c_k, Info_LineSearch] = self.LineSearch_Merit(beta, s, sigma, ...
    ...
    if self.Option.printLevel == 2
        ...
        prevIterMsg = ['  ',...
            ...
            num2str(max(norm(PSI, inf)), '%10.2e'), ' | ',...
