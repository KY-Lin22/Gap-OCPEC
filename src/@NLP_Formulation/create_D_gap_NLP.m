...
Dim.z_Node = cumsum([OCPEC.Dim.x, OCPEC.Dim.u, OCPEC.Dim.lambda, eta_Dim]); 
Dim.z = size(z, 1);
...
h_stage = [...
    ...
    F_stage - ETA;...
    C_stage];
h = reshape(h_stage, (OCPEC.Dim.x + eta_Dim + OCPEC.Dim.C) * OCPEC.nStages, 1);
Dim.h_Node = cumsum([OCPEC.Dim.x, eta_Dim, OCPEC.Dim.C]);
Dim.h = size(h, 1);
