function nlp = create_primal_gap_NLP_omega(self, OCPEC)
% formulate a relax generalized primal gap constraint based NLP (omega_c is a explicit variable)
%
% OCPEC has the form:
...
%            z = [z_1;...z_n;...z_N] and z_n = [x_n; u_n; lambda_n; eta_n; omega_c_n] 
%            with x_n:       system state
%                 u_n:       control                 
%                 lambda_n:  algebraic variable   
%                 eta_n:     auxiliary variable for VI function F in gap function
%                 omega_c_n: intermedia variable to evaluate primal gap function
...
%            h = [h_1;...h_n;...h_N] and h_n = [f_n; F_n - eta_n; omega_c_n - omega_c_func_n; C_n]     
%            with f_n:            discretized state equation f defined in OCPEC
%                 F_n:            VI function
%                 omega_c_func_n: omega_c explicit expression
...
%                 phi_n: generalized primal gap function (function of lambda, eta, and omega_c)
...
OMEGA_c = SX.sym('OMEGA_c', OCPEC.Dim.lambda, OCPEC.nStages);
...
% generalized primal gap function phi_c (function of lambda, eta, and omega_c)
param_c = 1;
phi_c_func = self.create_phi_c_func_omega(OCPEC, param_c);
phi_c_func_map = phi_c_func.map(OCPEC.nStages);
% omega_c_func
omega_c_func = self.create_omega_solver(OCPEC, param_c);
omega_c_func_map = omega_c_func.map(OCPEC.nStages);
...
C_stage = C_map(X, U);
% gap function
phi_stage = phi_c_func_map(LAMBDA, ETA, OMEGA_c);
omega_c_stage = omega_c_func_map(LAMBDA, ETA);

%% reshape NLP variable and function (column, lowercase)
% variable
Z = [X; U; LAMBDA; ETA; OMEGA_c];
z = reshape(Z, (OCPEC.Dim.x + OCPEC.Dim.u + 3* OCPEC.Dim.lambda) * OCPEC.nStages, 1);
Dim.z_Node = cumsum([OCPEC.Dim.x, OCPEC.Dim.u, OCPEC.Dim.lambda, OCPEC.Dim.lambda, OCPEC.Dim.lambda]); 
Dim.z = size(z, 1);
...
h_stage = [...
    ...
    F_stage - ETA;...
    OMEGA_c - omega_c_stage; ...
    C_stage];
h = reshape(h_stage, (OCPEC.Dim.x + OCPEC.Dim.lambda + OCPEC.Dim.lambda + OCPEC.Dim.C) * OCPEC.nStages, 1);
Dim.h_Node = cumsum([OCPEC.Dim.x, OCPEC.Dim.lambda, OCPEC.Dim.lambda, OCPEC.Dim.C]);
Dim.h = size(h, 1);
